...
DEFAULT_RESULTS_PATH = '../Results/SingleTask/'
DEFAULT_DATASETS_PATH = '../Final_CSVs/'
DEFAULT_FIGURES_PATH = '../Results/SingleTask/'
...
class TensorFlowSTLWrapper:
	...
	def __init__(self, dataset_name, target_label, users_as_tasks=True, test_steps=9001, val_output_file=None, 
		...
		self.wanted_feats = [x for x in self.data_df.columns.values if x != 'id' and x != 'timestamp' and x!= 'dataset' and 'stress' not in x]
		if self.users_as_tasks:
			self.wanted_labels = [target_label]
			self.n_tasks = len(self.data_df['id'].unique())
		else:
			self.wanted_labels = [x for x in self.data_df.columns.values if 'stress' in x and 'tomorrow_' in x and 'Evening' in x and 'Alertness' not in x and 'Energy' not in x]
		...
		self.decay = [True]
		self.decay_steps = [1000]#[10000]
		self.decay_rates = [0.95]
		self.optimizers = [tf.train.AdamOptimizer] #[tf.train.AdagradOptimizer,  tf.train.GradientDescentOptimizer
		self.train_steps =[2]#[4001]
		self.batch_sizes = [64]#[5,10,20]
		self.learning_rates = [.01]#[.01, .001]
		self.architectures = [[50,50]]#[[100],[50,5],[100,10]] if architectures is None else architectures
	...
	def sweepParametersForOneTask(self, task_name, target_label):
		if self.users_as_tasks:
			task_df = self.data_df[self.data_df['id'] == task_name]
		else:
		...
		print(best_results_dict.keys())
		test_acc, test_auc, test_f1, test_preds = self.getFinalResultsForTask(best_results_dict)
		print("FINALTEST", self.net.final_test_results)
		...
		best_results_dict['test_f1'] = test_f1
	...
	def getFinalResultsForTask(self, setting_dict):
		...
		if self.users_as_tasks:
			task_df = self.data_df[self.data_df['id'] == setting_dict['task_name']]
		...
		preds_df.to_csv(self.results_path + "Preds-" + self.save_prefix + str(label_name) + '.csv')
		print ("Preds df saved to", self.results_path + "Preds-" + self.save_prefix + str(label_name) + '.csv')

		return self.net.final_test_results['acc'][0], self.net.final_test_results['auc'][0], self.net.final_test_results['f1'][0], preds
	...
	def testOneSettingForOneTask(self, hidden_layers, l2_beta, lrate, dropout, decay, dsteps, drate, bsize, opt, tsteps):
		...
		results_dict = {'hidden_layers':hidden_layers, 'l2_beta': l2_beta, 'learning_rate': lrate, 
						...
						'optimizer': opt, 'val_acc': val_acc, 'val_auc':val_auc, 'val_f1':val_f1}
	...
	def run(self):
		...
		if self.users_as_tasks:
			...
			print ("\tValidation set: Accuracy =", np.nanmean(self.val_results_df['val_acc']), "AUC = ", np.nanmean(self.val_results_df['val_auc']), "F1 = ", np.nanmean(self.val_results_df['val_f1']))
			print ("\tTest set: Accuracy =", np.nanmean(self.val_results_df['test_acc']), "AUC = ", np.nanmean(self.val_results_df['test_auc']),"F1 = ", np.nanmean(self.val_results_df['test_f1']))
			print ("")
...
if __name__ == "__main__":
	...
	print ("\tFor each task individually, this code will sweep a set of network architectures and parameters to find the ideal settings")
