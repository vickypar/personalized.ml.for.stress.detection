...
class TensorFlowWrapper:
	def __init__(self, dataset_name, target_label=None, trial_name=None, multilabel=False, multitask=False, 
		...
		self.decay = [True]
		self.decay_steps = [100] #100
		self.decay_rates = [0.95]
		self.optimizers = [tf.train.AdamOptimizer] #[tf.train.AdagradOptimizer,  tf.train.GradientDescentOptimizer
		self.train_steps = [300]#[5001]#300
		if multitask:
			self.batch_sizes = [250, 150, 90]#[64]#[20] 30 70 50 80 100 800, 900
			self.learning_rates = [.01, .001]#[.01]
			self.architectures = [[25, 10, 5], [10, 15, 10], [30, 20, 30]] if architectures is None else architectures # [100, 50, 40], [30, 20, 10], [100, 50], [150, 150], [10,10], [20,20], [50, 50], [40,40], [45,45], [15,10], [10,15,10] 
		else:
			self.batch_sizes = [50,75]
			self.learning_rates = [.01, .001, .0001]
			self.architectures = [[50, 50],[40,40]] if architectures is None else architectures #[10,15,10],[500,50],[20,20]
	...
	def run(self):
		...
		self.findBestSetting(optimize_for='val_f1')
